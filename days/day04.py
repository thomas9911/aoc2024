...
for (y, line) in enumerate(lines):
  for (x, val) in enumerate(line.elems()):
    if val == 'A':
      one = "".join([map_getter(line_map, x-1, y-1), map_getter(line_map, x, y), map_getter(line_map, x+1, y+1)])
      two = "".join([map_getter(line_map, x+1, y-1), map_getter(line_map, x, y), map_getter(line_map, x-1, y+1)])
